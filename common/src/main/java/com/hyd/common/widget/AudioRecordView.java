...
public class AudioRecordView extends FrameLayout implements View.OnTouchListener {
    ...
    private void turnRecord() {
        if (mIsRecording) {
            mPlayButton.animate().alpha(MIN_ALPHA).scaleX(1).scaleY(1).setDuration(320).setInterpolator(new AnticipateOvershootInterpolator()).start();
            mDeleteButton.animate().alpha(MIN_ALPHA).scaleX(1).scaleY(1).setDuration(320).setInterpolator(new AnticipateOvershootInterpolator()).start();
        } else {
            mPlayButton.animate().alpha(0).scaleX(0).scaleY(0).setDuration(260).setInterpolator(new DecelerateInterpolator()).start();
            mDeleteButton.animate().alpha(0).scaleX(0).scaleY(0).setDuration(260).setInterpolator(new DecelerateInterpolator()).start();
        }
    }
    ...
    private void onStop(boolean isCancel) {
        ...
        Callback callback = mCallback;
        if (callback != null) {
            callback.requestStopRecord(isCancel ? END_TYPE_CANCEL : (mActiveView == null ?
                    END_TYPE_NONE :
                    (mActiveView == mPlayButton ? END_TYPE_PLAY : END_TYPE_DELETE)));
        }
    }

    // 监听用户的手指触摸，并响应对应的开始，停止，取消等操作
    @Override
    ...
    private void refreshAlpha(boolean inLeft, double spaceLen) {
        ...
        int actionTintColor = overFlowIcon ? UiCompat.getColor(getResources(),
                R.color.colorAccent) : UiCompat.getColor(getResources(), R.color.textPrimary);
